...
const HUMAN_EDITOR_RULES = `按资深公众号主编的真实工作方式写：先判断读者为什么会点开、为什么会读下去，再组织信息。标题使用普通人会说、编辑敢发布的中文，优先具体对象、真实冲突、反常识或明确收益；除非主题确实是 AI，否则禁止把标题写成“某事：AI 如何……”。面向读者的标题、摘要、小标题和正文禁止使用日式直角引号「」或『』；确有引用必要时统一使用中文双引号“”，产品名和普通词语不必刻意加引号。不要为了显得深刻而滥用冒号、引号、“从 A 到 B”、“不是……而是……”和口号。正文避免“在当今快速发展的时代”“随着时代的发展”“值得注意的是”“不难发现”“综上所述”“总而言之”等模型套话，少用赋能、重塑、闭环、底层逻辑、时代浪潮等抽象词。允许长短句不齐、短段停顿和有分寸的口语；每一段必须带来事实、动作、场景或新的判断，不做同义反复。`;
